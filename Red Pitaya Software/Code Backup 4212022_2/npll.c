...
static int write_spi(void *write_data, int size);
...
int main(void){
  ...
  void* data3 = (void*)&hex3;
	...
	uint32_t hex4 = 0x58020006;
	void* data4 = (void*)&hex4;

  uint32_t hex5 = 0x2018000a;
  void* data5 = (void*)&hex5;

	uint32_t hex6 = 0x2018000a;
	void* data6 = (void*)&hex6;

  uint32_t hex7 = 0x309f000a;
  void* data7 = (void*)&hex7;

	uint32_t hex8 = 0x309f000a;
	void* data8 = (void*)&hex8;

  uint32_t hex9 = 0x0040000a;
  void* data9 = (void*)&hex9;

	uint32_t hex10 = 0x0040000a;
	void* data10 = (void*)&hex10;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(write_spi(data6, 4) < 0){
	...
	if(write_spi(data7, 4) < 0){
          printf("Write to SPI failed. Error: %s\n", strerror(errno));
          return -1;
  }

	if(write_spi(data8, 4) < 0){
          printf("Write to SPI failed. Error: %s\n", strerror(errno));
          return -1;
  }

	if(write_spi(data9, 4) < 0){
          printf("Write to SPI failed. Error: %s\n", strerror(errno));
          return -1;
  }

	if(write_spi(data10, 4) < 0){
          printf("Write to SPI failed. Error: %s\n", strerror(errno));
          return -1;
  }
...
static int write_spi(void *write_buffer, int size){
